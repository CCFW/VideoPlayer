...
//    qDebug() << "u_name:"+m_name;
    ...
    QSqlQuery ql;
    QString strql1 = QString("insert into Discuss(d_id,u_id,d_comment,u_name) values('%1','%2','%3','%4')").arg(id).arg(u_id).arg(comments).arg(u_name);
    ql.exec(strql1);

}

//获取所有的评论
QString Discuss::getComments()
{
    QSqlQuery sqlQuery;
    QString strql1 = "select d_comment from Discuss";
    sqlQuery.exec(strql1);
...
//获取所有评论过的昵称
QString Discuss::getName()
{
    QSqlQuery sqlQuery;
    QString strql1 = "select d_comment, u_name from Discuss";
    sqlQuery.exec(strql1);
