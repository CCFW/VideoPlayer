...
void Discuss::danMu(QString comment){
    ...
    QString strql = QString("insert into Discuss(d_id,u_id, d_danMu,u_name) values('%1','%2','%3','%4')").arg(id).arg(u_id).arg(comment).arg(m_name);
    ql.exec(strql);
}

void Discuss::pingLun(QString pinglun)
{
    int id = 0;
    ...
    qDebug() << id;
    qDebug() << u_id;
    qDebug() << "comment:"+pinglun;
    QString strql = QString("insert into Discuss(d_id,u_id, d_comment,u_name) values('%1','%2','%3','%4')").arg(id).arg(u_id).arg(pinglun).arg(m_name);
    ql.exec(strql);
}
